...
public class main1 extends javax.swing.JFrame {
    ...
    private JTextPane buildTerminal;
    ...
    public main1() {
        ...
        inicializarTerminal();
        resetApp();
        panelCodigoMaquina1.setOutputCallback(this::appendBuildTab);
    }
    ...
    private void resetApp() {
        ...
        if (buildTerminal != null) buildTerminal.setText("");
    }
    ...
    /** Forwards build output from PanelCodigoMaquina to a "Build" tab in jTabbedPaneTerminal. */
    private void appendBuildTab(String texto, Color color) {
        if (buildTerminal == null) {
            buildTerminal = new JTextPane();
            buildTerminal.setEditable(false);
            buildTerminal.setBackground(new Color(30, 37, 40));
            buildTerminal.setForeground(Color.WHITE);
        }

        // Show terminal area
        jSplitPanelVertical.setEnabled(true);
        jSplitPanelVertical.setDividerSize(4);
        if (jSplitPanelVertical.getDividerLocation() < 50) {
            jSplitPanelVertical.setDividerLocation(0.7);
        }

        // Find or create the Build tab
        int buildIdx = -1;
        for (int i = 0; i < jTabbedPaneTerminal.getTabCount(); i++) {
            if ("Build".equals(jTabbedPaneTerminal.getTitleAt(i))) {
                buildIdx = i;
                break;
            }
        }
        if (buildIdx < 0) {
            JScrollPane sp = new JScrollPane(buildTerminal);
            sp.setBorder(null);
            jTabbedPaneTerminal.addTab("Build", sp);
            buildIdx = jTabbedPaneTerminal.getTabCount() - 1;
        }
        jTabbedPaneTerminal.setSelectedIndex(buildIdx);

        appendTerminal(buildTerminal, texto, color);
    }
    ...
    private void jButtonRunActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonRunActionPerformed
        ...
        try {
            ...
            // Optimization: run optimizer and show log in terminal
            panelCodigoOptimizado1.showOptimizado(code);
            List<String> optLog = panelCodigoOptimizado1.getLog();
            for (String linea : optLog) {
                if (linea.startsWith("===")) {
                    appendNormal(terminal, linea);
                } else if (linea.contains("→")) {
                    appendTerminal(terminal, linea, new Color(130, 210, 255));
                } else if (linea.startsWith("  (ninguna")) {
                    appendTerminal(terminal, linea, new Color(150, 150, 150));
                } else if (!linea.isEmpty()) {
                    appendTerminal(terminal, linea, new Color(220, 200, 100));
                } else {
                    appendNormal(terminal, "");
                }
            }

            // Machine code: pass already-optimized TAC so we don't re-optimize
